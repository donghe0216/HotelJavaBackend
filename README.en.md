...
Spring Boot REST API for a hotel booking system, built as a QA portfolio project demonstrating two-layer test architecture, state machine testing, and security boundary validation.
...
| `api/AuthorizationTest` | TC-AUTH-* | 401/403 boundary tests |
| `api/RoomApiTest` | TC-R-* | Room management endpoints |
| `api/UserApiTest` | TC-U-* | User management endpoints |
...
Bugs are intentionally preserved as portfolio material.

| ID | Location | Description |
|----|----------|-------------|
| TC-B-05 | `BookingServiceImpl` | Date validation does not reject `checkIn == checkOut` — condition compares date against itself |
| TC-AUTH-PAY | `PUT /payments/update` | Endpoint has no auth guard — accepts requests from anonymous users |
